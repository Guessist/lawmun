...
function openFlyout($btn, $menu){
  const rect = $btn[0].getBoundingClientRect();

  // 메뉴 위치: 버튼 왼쪽에 맞추고, topbar 아래로
  const topbarHeight = 56;
  const left = Math.max(8, Math.min(rect.left, window.innerWidth - $menu.outerWidth() - 8));
  const top = topbarHeight + 6; // 살짝 띄우기

  $menu.css({ left: left + "px", top: top + "px" }).show();
  $btn.attr("aria-expanded", "true");
}

function closeFlyout($btn, $menu){
  $menu.hide();
  $btn.attr("aria-expanded", "false");
}

$(function(){
  const $btn = $("#overseasBtn");
  const $menu = $("#overseasFlyout");
  ...
  $btn.on("click", function(e){
    ...
    if($menu.is(":visible")){
      closeFlyout($btn, $menu);
    } else {
      openFlyout($btn, $menu);
    }
  });

  // 메뉴 클릭은 닫히지 않게(링크 이동은 그대로)
  ...
  $(document).on("keydown", function(e){
    if(e.key === "Escape") closeFlyout($btn, $menu);
  });

  // 스크롤/리사이즈 시 닫기(위치 틀어짐 방지)
